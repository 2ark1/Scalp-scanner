...
Rules: Location (Weekly Close / 20-50 EMA / 4H S/D Zone) → BOS (4H) + TFCOT (2H) → Precision (0.618 Fib / Vol POC)
...
def check_location(current_price, ema20, ema50, weekly_close, sd_zones, bos_direction="None"):
    """
    Location criteria — price must be near ANY of:
      1. Previous weekly close (within 0.33%)
      2. Daily 20 EMA (within 0.33%)
      3. Daily 50 EMA (within 0.33%)
      4. Inside a 4H S/D zone (RBR, DBD, RBD, DBR)
    Directional filter applied where possible.
    Returns: (hit: bool, reasons: list)
    """
    EMA_TOL = 0.33  # % tolerance
    hits = []

    # ── 1. Weekly close
    if weekly_close:
        wc_pct = abs(current_price - weekly_close) / weekly_close * 100
        if wc_pct <= EMA_TOL:
            side = "above" if current_price > weekly_close else "below"
            hits.append(f"Prev Week Close @ {weekly_close:.4f} ({side}, {wc_pct:.2f}% away)")

    # ── 2. Daily 20 EMA
    if ema20:
        ema20_pct = abs(current_price - ema20) / ema20 * 100
        if ema20_pct <= EMA_TOL:
            side = "above" if current_price > ema20 else "below"
            role = "support" if bos_direction == "Bullish" else ("resistance" if bos_direction == "Bearish" else "level")
            hits.append(f"Daily 20 EMA @ {ema20:.4f} ({side}, {ema20_pct:.2f}% away) — {role}")

    # ── 3. Daily 50 EMA
    if ema50:
        ema50_pct = abs(current_price - ema50) / ema50 * 100
        if ema50_pct <= EMA_TOL:
            side = "above" if current_price > ema50 else "below"
            role = "support" if bos_direction == "Bullish" else ("resistance" if bos_direction == "Bearish" else "level")
            hits.append(f"Daily 50 EMA @ {ema50:.4f} ({side}, {ema50_pct:.2f}% away) — {role}")

    # ── 4. 4H S/D zones (price inside zone)
    for z in (sd_zones or []):
        if z['bottom'] <= current_price <= z['top']:
            zone_type = z.get('type', '')
            # Directional filter: demand zones for bullish, supply for bearish
            if bos_direction == "Bullish" and "Supply" in zone_type:
                continue
            if bos_direction == "Bearish" and "Demand" in zone_type:
                continue
            hits.append(
                f"4H {zone_type} zone {z['bottom']:.4f}–{z['top']:.4f} "
                f"({z.get('strength','')}, {z.get('timeframe','')})"
            )
...
def detect_sd_zones(df_4h, df_weekly, current_price, lookback=120):
    """
    ...
    zones += find_zones_in_df(df_4h,    "4H",     min_impulse_pct=0.3, max_base_range_pct=0.25)
...
def scan_instrument(symbol, info):
    ...
    # HTF Levels (kept for display in expandable section)
    htf_levels = detect_htf_levels(df_daily, df_weekly, df_monthly, current_price)
    result["htf_levels"] = htf_levels

    # Weekly close (prev week's close for location check)
    weekly_close = None
    if df_weekly is not None and len(df_weekly) >= 2:
        weekly_close = round(float(df_weekly['Close'].dropna().iloc[-2]), 4)
    result["weekly_close"] = weekly_close

    # 4H S/D Zones — part of location criteria
    sd_zones = detect_sd_zones(df_4h, df_weekly, current_price)
    result["sd_zones"] = sd_zones

    # BOS (4H) + TFCOT (2H) — run before location so direction feeds in
    ...
    # Location — weekly close, daily 20/50 EMA (0.33%), 4H S/D zones
    location_hit, location_reasons = check_location(
        current_price, ema20, ema50, weekly_close, sd_zones, bos_direction=bos_direction
    )
    result["location_hit"]     = location_hit
    result["location_reasons"] = location_reasons

    # Precision — 0.618 Fib from 2H impulse + Vol POC confluence
    if impulse_low and impulse_high and impulse_high > impulse_low:
        swing_low, swing_high = impulse_low, impulse_high
    elif bos_swing_low and bos_swing_high and bos_swing_high > bos_swing_low:
        swing_low, swing_high = bos_swing_low, bos_swing_high
    else:
    ...
    # Precision items: 0.618 Fib is trigger, Vol POC is confluence
...
def build_report(results):
    ...
    lines.append("            Weekly Close / 20-50 EMA / 4H S/D Zone  |  4H BOS + 2H TFCOT  |  0.618 Fib / Vol POC")
    lines.append("")
