...
def compute_avwap(df, anchor_bars=20):
    ...
    avwap = (sub['TPV'].cumsum() / sub['Volume'].cumsum()).iloc[-1]
    return round(float(avwap), 4)
...
def scan_instrument(symbol, info):
    ...
    avwap = compute_avwap(df_daily, anchor_bars=20)
    ...
    result["precision_hit"]  = len(near_precision) > 0
